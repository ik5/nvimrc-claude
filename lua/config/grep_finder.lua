-- Patched snacks grep finder: upstream spams notify.error for ripgrep lines it
-- cannot parse (-v uses line:-:text; some lines use line:text without a column).
...
---@return string?, string?, string? line, col, text
local function parse_rest(rest)
	local line, col, text = rest:match("^(%d+):([^:]+):(.*)$")
	if line and col then
		if col == "-" then
			col = "1"
		elseif not col:match("^%d+$") then
			-- e.g. 205:end (no column) — middle field is match text
			return line, "1", col .. (text ~= "" and (":" .. text) or "")
		end
		return line, col, text or ""
	end
	line, text = rest:match("^(%d+):(.*)$")
	if line then
		return line, "1", text or ""
	end
	return nil
end
...
---@param cwd string?
---@return snacks.picker.finder.Item|false
local function transform(item, cwd)
	item.cwd = cwd

	local file, rest
	local file_sep = item.text:find("\0")
	if file_sep then
		file = item.text:sub(1, file_sep - 1)
		rest = item.text:sub(file_sep + 1)
	else
		file, rest = item.text:match("^(.-):(%d+:.+)$")
		if not file then
			return false
		end
	end

	local line, col, text = parse_rest(rest)
	if not line then
		return false
	end

	item.text = file .. ":" .. rest:gsub(MATCH_SEP, "")
	local from = tonumber(col) or 1
...
function M.grep(opts, ctx)
	...
	if opts.debug and opts.debug.grep then
